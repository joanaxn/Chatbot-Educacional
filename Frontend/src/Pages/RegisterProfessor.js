...
function RegisterProfessor() {
  ...
  const handleSubmit = (e) => {
    ...
    if (professores.find((p) => p.email === form.email)) {
      alert('Já existe um professor com este email.');
      return;
    }
    professores.push(form);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6 flex justify-center items-center">
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-xl space-y-6">
        <h2 className="text-2xl font-bold text-blue-700">Registo de Professor</h2>
        ...
        {sucesso && <p className="text-green-600 font-medium">✅ Professor registado com sucesso!</p>}
      </form>
    </div>
  );
}
